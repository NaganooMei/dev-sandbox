...
REPORT_PATH = DOCS_DIR / "ascend_h2d_glm512_ce_ffts_matrix_report.md"
DATA_PATH = DOCS_DIR / "data" / "ascend_h2d_glm512_144_matrix.tsv"
FULL_DATA_PATH = DOCS_DIR / "data" / "ascend_h2d_glm512_144_matrix_full.tsv"
...
![CE shared Host memory absolute WallBW](images/{images[("ce", "one_share")]})

### 3.2 FFTS · 共享 Host 内存

![FFTS shared Host memory absolute WallBW](images/{images[("ffts", "one_share")]})

### 3.3 CE · 非共享 Host 内存

![CE non-shared Host memory absolute WallBW](images/{images[("ce", "all")]})

### 3.4 FFTS · 非共享 Host 内存

![FFTS non-shared Host memory absolute WallBW](images/{images[("ffts", "all")]})
...
多卡 Submit/Copy 时间先按每轮所有设备进程中的最大值合并，再对 128 轮计算统计量。STREAM 模式不使用 device Event 统计 Copy 时间，因此 Copy 和 DevBW 显示为 N/A；它仍然通过 GroupWall 和 WallBW 记录端到端完成时间。原始汇总保存在 `docs/data/ascend_h2d_glm512_144_matrix.tsv`，从完整日志解析出的全量数据保存在 `docs/data/ascend_h2d_glm512_144_matrix_full.tsv`。
