...
| 对比对象 | FFTS 1MiB pipeline、FFTS 2MiB pipeline、FFTS 全聚合不 pipeline、Ascend multi-stream CE |
...
- multi-stream CE 仍然是逐 fragment copy；FFTS pipeline 是每卡先聚合到 staging slot，再 split 到 device fragments。
